...
class MSWindow {
  constructor(options = {}) {
    this.id = options.id || 'window_' + Math.random().toString(36).substr(2, 9);
...
function toggleChatLog() {
  const chatWindow = document.getElementById('chatLogWindow');
...
function setup() {
    ...
    bonzi.$dialog
        ...
        .html(`<img src="${data.url}" style="max-width: 300px; max-height: 300px;">`)
        .show();
});
...
socket.on("video", function(data) {
    ...
    bonzi.$dialog
        .addClass("bubble_autowidth")
        .html(`<video controls style="max-width: 300px; max-height: 300px;"><source src="${data.url}"></video>`)
        .show();
});
...
socket.on("audio", function(data) {
    ...
    bonzi.$dialog
        .addClass("bubble_autowidth") 
        .html(`<audio controls><source src="${data.url}"></audio>`)
        .show();
}); 
        socket.on("talk", function (a) {
            var b = bonzis[a.guid];
            const chatLogContent = document.getElementById('chatLogContent');
            const message = `${usersPublic[a.guid].name}: ${a.text}`;
            chatLogContent.innerHTML += `<div>${message}</div>`;
        ...
        socket.on("announcement", function(a) {
            new MSWindow({
            title: `Announcement from ${a.name}`,
            width: 400,
            height: 150,
            content: `<p style="margin: 0; padding: 10px;">${a.text}</p>`
            });
        }),
    ...
    Bonzi = (function () {
        ...
        return (
                        ...
                        if (!this.mute) {
                            ...
                            this.$dialog
                                ...
                                .html(
                                    "\n\t\t\t\t\t<" +
                                        b +
                                        ' type="text/html" width="480" height="270" src="https://www.youtube.com/embed/' + [VIDEO_ID] + '?autoplay=1&controls=1&loop=0&mute=0"
            ...
            value: function(a) {
                this.runSingleEvent([{
                    type: "text",
                    text: a + "STOP BEING A KIDDIE",
                    say: a + "STOP BEING A KIDDIE"
                }])
            }
        },
                {
    ...
    window.BonziHandler = new (function () {
        return (
            ...
            (this.prepSprites = function () {
                for (var a = ["black", "blue", "brown", "green", "purple", "red", "pope", "clippy", "peedy"], b = 0; b < a.length; b++) {
                    var c = a[b],
...
var loadQueue = new createjs.LoadQueue(),
    loadDone = [],
    loadNeeded = ["bonziBlack", "bonziBlue", "bonziBrown", "bonziGreen", "bonziPurple", "bonziRed", "clippy", "peedy", "topjej"];
...
$(function () {
    $("#login_go").click(loadTest),
        ...
        socket.on("kick", function(a) {
            $("#page_kick").show(), $("#ban_reason").html(a.reason)
        }),
